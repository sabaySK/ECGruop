...
<main class="pt-90">
                  ...
                  @foreach ($categories as $category)
                    <li class="list-item">
                      <span class="menu-link py-1">
                        <input type="checkbox" name="categories" id="" value="{{$category->id}}" class="chk-brand"
              ...
              <div class="search-field multi-select accordion-body px-0 pb-0">
                <ul class="list list-inline mb-0 brand-list">
                  @foreach ($brands as $brand)
                      <li class="list-item">
                        <span class="menu-link py-1">
                          <input type="checkbox" name="brands" id="" value="{{$brand->id}}" class="chk-brand"
      ...
      <div class="shop-list flex-grow-1">
        ...
        <div class="d-flex justify-content-between mb-4 pb-md-2">
          ...
          <div class="shop-acs d-flex align-items-center justify-content-between justify-content-md-end flex-grow-1">
            <select class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0" aria-label="Page size" name="pagesize" id="pagesize" style="margin-right: 20px">
              <option value="12" {{$size == 12 ? 'selected' : ""}} selected>Show</option>
              <option value="24" {{$size == 24 ? 'selected' : ""}} selected>24</option>
              <option value="48" {{$size == 48 ? 'selected' : ""}} selected>48</option>
              <option value="102" {{$size == 102 ? 'selected' : ""}} selected>102</option>
            </select>
                    ...
                    <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                      <div class="pc__img-wrapper">
                        <div class="swiper-container background-img js-swiper-slider" data-settings='{"resizeObserver": true}'>
                          <div class="swiper-wrapper">
                            <div class="swiper-slide">
                              <a href="{{route('shop.product_details', ['product_slug'=>$product->slug])}}"><img loading="lazy" src="{{asset('uploads/products')}}/{{$product->image}}" width="330" height="400" alt="{{$product->name}}" class="pc__img"></a>
                            </div>
                            <div class="swiper-slide">
                              @foreach (explode(",", $product->images) as $gimg)
                                <a href="{{route('shop.product_details', ['product_slug'=>$product->slug])}}"><img loading="lazy" src="{{asset('uploads/products/thumbnail')}}//{{trim($gimg)}}" width="330" height="400" alt="{{$product->name}}" class="pc__img"></a>
                              @endforeach
                            </div>
                          </div>
                        ...
                        <form name="addtocart-form" method="post" action="{{route('cart.add')}}">
                          ...
                          <input type="hidden" name="price" id="" value="{{$product->sale_price == '' ? $product->regular_price : $product->sale_price}}">
                        <button type="submit" class="pc__atc btn anim_appear-bottom btn position-absolute border-0 text-uppercase fw-medium js-add-cart" data-aside="cartDrawer" title="Add To Cart">Add To Cart</button>
                        </form>
                        @endif
                      </div>
  ...
  <form action="{{route('shop.index')}}" id="formfilter" method="GET">
    @csrf
